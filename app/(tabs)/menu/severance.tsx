...
import { calculateSeverancePay, SEVERANCE_PAY_CEILING_2026 } from '../../../src/utils/benefitsCalculations';
import { formatTL } from '../../../src/utils/salary';

export default function SeveranceCalculatorScreen() {
  ...
  const handleCalculate = () => {
    ...
    if (y === 0 && m === 0 && d === 0) {
      toast.warning(language === 'tr' ? 'Lütfen çalıştığı süreyi girin' : 'Please enter tenure duration');
      return;
    }
      ...
      <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
        ...
        {result && (
          <View style={[styles.resultBox, { backgroundColor: colors.surface }]}>
            ...
            <View style={styles.resultRow}>
              <Text style={[styles.resultLabel, { color: colors.textMuted }]}>
                {language === 'tr' ? 'Brüt Tazminat' : 'Gross Severance'}
              </Text>
              <Text style={[styles.resultAmount, { color: colors.text }]}>
                {formatTL(result.grossSeverance)}
              </Text>
            </View>
